...
void main() {
  ...
  testWidgets('renders the default label and triggers onPressed on tap', (
    ...
    await tester.pumpWidget(
      host(KakaoSignInButton(onPressed: () => taps += 1)),
    );

    expect(find.text('카카오 로그인'), findsOneWidget);
    await tester.tap(find.byType(KakaoSignInButton));
    expect(taps, 1);
  });
  ...
  testWidgets('shows a spinner and ignores taps while loading', (
    ...
    var taps = 0;
    await tester.pumpWidget(
      host(KakaoSignInButton(onPressed: () => taps += 1, isLoading: true)),
    );
  ...
  testWidgets('is disabled when isDisabled is true', (final tester) async {
    var taps = 0;
    await tester.pumpWidget(
      host(KakaoSignInButton(onPressed: () => taps += 1, isDisabled: true)),
    );
